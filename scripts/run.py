...
def main() -> None:
    ...
    workdir = Path(args.workdir).expanduser().resolve() if args.workdir else path(config, agent_config["workdir"])
    trajectory = Path(args.trajectory).expanduser().resolve() if args.trajectory else path(config, agent_config["trajectory"])
    memory = None
    ...
    experience = None if args.no_experience else ExperienceLog(path(config, agent_config.get("experience_dir", "state/experience")))
    skills = None if args.no_skills else SkillRegistry(path(config, agent_config.get("skills_dir", "skills")))
