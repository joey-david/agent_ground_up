...
def test_crop_middle_preserves_both_ends() -> None:
    ...
    assert crop_middle(text, 5).splitlines() == [
        "line 0",
        "line 1",
        "... 6 lines omitted ...",
        "line 8",
...
def test_tui_streams_text_then_prints_tool_call() -> None:
    stream = StringIO()
    ...
    ui.token("first ")
    ui.token("token")
    ui.end_assistant({"content": "first token", "tool_calls": [call]})
    ui.tool("bash", "diff --git a/a.py b/a.py\n-old = 1\n+new = 2")
    ui.status("status=completed")

    output = stream.getvalue()
    for text in ("User", "Agent", "first token", "→ bash", "Tool · bash", "status=completed"):
